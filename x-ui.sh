...
cleanup_ssh_manager() {
    ...
    # UDPGW is managed by a 4x-ui-owned systemd template. Stop only the ports
    # recorded by the panel; never kill an administrator's unrelated badvpn.
    local udpgw_state="/etc/x-ui/ssh-manager/udpgw-ports"
    if command -v systemctl >/dev/null 2>&1 && [[ -f "$udpgw_state" ]]; then
        while read -r udpgw_port; do
            [[ "$udpgw_port" =~ ^[0-9]+$ ]] || continue
            systemctl disable --now "xui-udpgw@${udpgw_port}.service" 2>/dev/null || true
        done < "$udpgw_state"
    fi
    rm -f /etc/systemd/system/xui-udpgw@.service "$udpgw_state"

    # 4x-ui may have masked Ubuntu's broken sshd-socket-generator when the
    # installed OpenSSH package cannot parse Match LocalPort. Remove only a
    # mask that 4x-ui itself recorded as owned; never touch an administrator's
    # pre-existing generator override.
    local ssh_generator_mask="/etc/systemd/system-generators/sshd-socket-generator"
    local ssh_generator_marker="/etc/x-ui/ssh-manager/sshd-socket-generator.masked-by-4x-ui"
    if [[ -f "$ssh_generator_marker" ]]; then
        if [[ -L "$ssh_generator_mask" ]] && [[ "$(readlink "$ssh_generator_mask" 2>/dev/null)" == "/dev/null" ]]; then
            rm -f "$ssh_generator_mask"
        fi
        rm -f "$ssh_generator_marker"
    fi

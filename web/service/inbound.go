...
func (s *InboundService) checkPortExist(listen string, port int, ignoreId int) (bool, error) {
	...
	// Hidden Payload Bypass backend ports are reserved even though they are
	// loopback-only. Rejecting them here avoids a new public inbound stealing
	// a port that Xray already uses behind another gateway.
	hidden := db.Model(model.Inbound{}).Where("payload_backend_port = ?", port)
	if ignoreId > 0 {
		hidden = hidden.Where("id != ?", ignoreId)
	}
	var hiddenCount int64
	if err := hidden.Count(&hiddenCount).Error; err != nil {
		return false, err
	}
	if hiddenCount > 0 {
		return true, nil
...
func (s *InboundService) AddInbound(inbound *model.Inbound) (*model.Inbound, bool, error) {
	...
	if err := s.ensurePayloadBypassConfig(inbound); err != nil {
		return inbound, false, err
	}
	...
	defer func() {
		if err == nil {
			...
			tx.Rollback()
			payloadBypassManager.Stop(inbound.Id)
		}
	}()
	...
	if err = payloadBypassManager.Reconcile(inbound); err != nil {
		return inbound, false, err
	}
...
func (s *InboundService) SetInboundEnable(id int, enable bool) (bool, error) {
	...
	if enable {
		...
		if inbound.PayloadBypass {
			if err := db.Model(model.Inbound{}).Where("id = ?", id).
				Update("payload_backend_port", inbound.PayloadBackendPort).Error; err != nil {
				_ = db.Model(model.Inbound{}).Where("id = ?", id).Update("enable", false).Error
				return false, err
			}
		}
...
func (s *InboundService) UpdateInbound(inbound *model.Inbound) (*model.Inbound, bool, error) {
	...
	originalInbound := *oldInbound
	if inbound.PayloadBypass && oldInbound.PayloadBypass && oldInbound.PayloadBackendPort > 0 {
		inbound.PayloadBackendPort = oldInbound.PayloadBackendPort
	}
	if err := s.ensurePayloadBypassConfig(inbound); err != nil {
		return inbound, false, err
	}
	...
	defer func() {
		if err != nil {
			tx.Rollback()
			_ = payloadBypassManager.Reconcile(&originalInbound)
		} else {
			tx.Commit()
		}
	}()
	...
	oldInbound.PayloadBypass = inbound.PayloadBypass
	oldInbound.PayloadBackendPort = inbound.PayloadBackendPort
	...
	s.xrayApi.Init(p.GetAPIPort())
	oldRuntimeDeleted := false
	if s.xrayApi.DelInbound(tag) == nil {
		logger.Debug("Old inbound deleted by api:", tag)
		oldRuntimeDeleted = true
	}

	// Reconcile the public Go listener only after the old Xray inbound is
	// removed. This is required when Payload Bypass is enabled on an existing
	// direct inbound because Xray previously owned the same public port.
	if err = payloadBypassManager.Reconcile(oldInbound); err != nil {
		_ = payloadBypassManager.Reconcile(&originalInbound)
		if originalInbound.Enable && oldRuntimeDeleted {
			if restored, restoreErr := s.buildRuntimeInboundForAPI(tx, &originalInbound); restoreErr == nil {
				if restoredJSON, marshalErr := json.MarshalIndent(restored.GenXrayInboundConfig(), "", "  "); marshalErr == nil {
					_ = s.xrayApi.AddInbound(restoredJSON)
				}
			}
		}
		s.xrayApi.Close()

...
func (s *PanelService) StartUpdate() error {
	...
	// Give the HTTP handler time to flush its JSON response before update.sh
	// stops this service. Bounded curl retries prevent a transient GitHub error
	// from turning into a partial or apparently frozen update.
	updateScript := fmt.Sprintf("set -o pipefail; sleep 2; %s -fLSs --connect-timeout 10 --max-time 120 --retry 3 --retry-delay 2 https://raw.githubusercontent.com/sharif102007/4x-ui/main/update.sh | %s", shellQuote(curl), shellQuote(bash))

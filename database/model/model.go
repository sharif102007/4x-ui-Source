...
type Inbound struct {
	...
	PayloadBypass      bool     `json:"payloadBypass" form:"payloadBypass" gorm:"default:false"`
	PayloadBackendPort int      `json:"payloadBackendPort" gorm:"default:0"`
}
...
func (i *Inbound) GenXrayInboundConfig() *xray.InboundConfig {
	listen := i.Listen
	port := i.Port
	if i.PayloadBypass && i.PayloadBackendPort > 0 {
		// The native payload gateway owns the public inbound port. Xray only
		// receives the post-handshake TCP stream on a loopback-only backend.
		listen = "127.0.0.1"
		port = i.PayloadBackendPort
	}

...
var (
	...
	// logBufferMu guards logBuffer. Every log call appends to it from whichever
	// goroutine emitted the message, while the web UI reads the whole buffer from
	// a request handler - an unsynchronized read/write pair on a shared slice.
	// The race detector never flagged this because no test logs concurrently.

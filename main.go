...
func runWebServer() {
	...
	// Give the GC a soft ceiling before anything starts allocating.
	service.ApplyRuntimeTuning()
	...
	// Must run before the web server and its jobs start: the IP-limit job reads
	// the access log this enables.
	service.EnsureXrayAccessLog()
